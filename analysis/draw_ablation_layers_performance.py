...
tune_layer_list=list(range(1,6))
marker_list=['o','s','d']
...
for mi, m in enumerate(wanted_metrics):
    ax1.plot(tune_layer_list,[metrics_list[i][m] for i in range(5)],
             label=wanted_metrics_alter_name[mi],
             )
...
ax1.set_ylim(8,49)
# 设置Y轴标签
ax1.set_xlabel('Fine-tune layers')
ax1.set_ylabel('BLEU-1')
plt.tight_layout()
plt.savefig('../figures/performance_with_layers.pdf')
# 显示图形
plt.show()