...
model.eval()
path='/hpc2hdd/home/yyang937/datasets/gwilliams2023/preprocess5/split2/interval_days.json'
with open(path, 'r', encoding='utf-8') as f:
    subj_interval_dict = json.load(f)
# 获取测试数据
subj_results_dict={}
for subj,interval in subj_interval_dict.items():
    print('subj',subj,interval)
    ...
    result_basename=f'formal_test_results_subj_{subj}'
    # 开始评估
    ...
    with open(json_file_path,'w') as f:
        json.dump(results,f)
    subj_results_dict[subj]=results


#画出来的指标有bleu-1到4，rouge，其他都暂时不画
eval_keys=['bleu-1','bleu-2','bleu-3','bleu-4','rouge1_fmeasure','rouge1_precision','rouge1_recall']
# 按照间隔天数排subj
sorted_dict = sorted(subj_interval_dict.items(), key=lambda x: x[1])
subjects = [item[0] for item in sorted_dict]
days=[subj_interval_dict[key] for key in subjects]
save_subjects_evaluation_data={
    'subj_results_dict':subj_results_dict,
    'days':days,
    'subjects':subjects,
    'eval_keys':eval_keys,
}

json_path=os.path.join(args.lora_model,'save_subjects_evaluation_data.json')
with open(json_path, "w") as f:
    json.dump(save_subjects_evaluation_data, f)
# 画图
plt.figure(figsize=(10,6))
for eval_key in eval_keys:
    plt.plot(days,[subj_results_dict[subj][eval_key] for subj in subjects])
plt.savefig(os.path.join(args.lora_model,'subjects_metrics.pdf'))
plt.close()
